...
for k, fname in enumerate(star_files):
    ...
    print(fname)
    data = hdul[0].data
    data[data>5800] = 0 #this removes bad pixels
    ...
    fig.savefig(paths.figures / f'psf_wing_vs_region_{k}.png', dpi=300)
    #plt.show()
    plt.close()
    ...
    table_out.write(paths.data / f"tables/wing_correction_{k}.csv", overwrite=True)
